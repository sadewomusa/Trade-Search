import { MARGIN_THRESHOLD, INDO_SIGNAL_WORDS, BLOCKED_SIGNALS, DISPOSABLE_DOMAINS } from "./constants";
...
// ══════════ SANITIZE ══════════
export function sanitizeIDR(price) {
...
// ══════════ CONFIDENCE ══════════
export function computeConfidence(results, priceStats) {
...
// ══════════ CLASSIFICATION ══════════
export function guessCategory(n) {
  const l = (n || "").toLowerCase();
  if (/phone|charger|cable|headphone|speaker|power bank|usb|bluetooth|watch/i.test(l)) return "electronics";
  if (/pan|pot|kitchen|cook|bake|knife|blender|mixer|plate|cutting.?board|talenan|chopping|peeler|grater|spatula|whisk|ladle|tong/i.test(l)) return "kitchen";
...
export function fallbackSearchQueries(productName, brand) {
  ...
  const idFull = clean.toLowerCase().split(/\s+/).map(w => EN_TO_ID[w] || w).join(" ");
  ...
  const queries = [];
  if (idCore) queries.push(idCore + addSpec);
  if (enCore) queries.push(enCore + addSpec);
  if (idShort.length) queries.push(idShort.join(" ") + addSpec);
  if (queries.length === 0) queries.push(clean.slice(0, 60));
  return { queries: [...new Set(queries)].slice(0, 5), cleanEn: enCore, cleanId: idCore };
}
...
export function detectBlockedSignals(rawText, platform) {
  for (const sig of BLOCKED_SIGNALS) { if (sig.pattern.test(rawText)) return platform + ": " + sig.reason; }
